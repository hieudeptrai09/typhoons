...
interface NamesByTagChartProps {
  names: TyphoonName[];
}

const NamesByTagChart = ({ names }: NamesByTagChartProps) => {
  const countMap: Record<string, { active: number; retired: number }> = {};
  names.forEach((n) => {
    const tag = n.tag || "Untagged";
    if (!countMap[tag]) countMap[tag] = { active: 0, retired: 0 };
    if (n.isRetired) {
      countMap[tag].retired += 1;
    } else {
      countMap[tag].active += 1;
    }
  });
...
export default NamesByTagChart;
