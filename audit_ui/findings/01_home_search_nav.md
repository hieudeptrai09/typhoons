...
- **Status:** ✅ **Fixed (keep-table affordance):** the results table keeps the list layout on mobile (better for comparing rows) and is now navigable — `sticky` header, `fixed:"left"` `#`/`Name` columns, sortable headers, and (via the shared `TableScrollHint`) a mobile "Swipe right →" hint plus a right-edge gradient fade, so the off-screen Storms/Replacement/Note columns are discoverable.
