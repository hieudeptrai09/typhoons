...
- **Status:** ✅ **Fixed (bdf8aa0):** `activeFilterCount` now only counts `selectedStatus` when `showHistory` is true (`settings.showHistory ? selectedStatus : ""`), so the funnel no longer shows a phantom "1" on the default view — verified in the rebuilt app (no badge).
...
- **Status:** ✅ **Fixed (keep-grid affordance):** `PositionNameGrid` renders through the shared `PositionGrid` component, which now shows the mobile "Swipe right to see full table →" hint **and** a right-edge gradient fade (`bg-gradient-to-l from-stone-100`, `md:hidden`), so the clipped columns are clearly signalled on mobile. The owner keeps the grid layout on both breakpoints (a shared row/column layout makes cross-country comparison and per-position counting easier), so the fix is the scroll affordance rather than a card transform. (Defaulting `showLetterNav` to true and a sticky first column remain optional polish.)
...
- **Fix (per owner — keep the grid on both breakpoints):** The owner keeps the country grid on mobile as well as desktop (the shared 2D position×country matrix is what makes cross-country comparison and per-position counting easy), so instead of a card transform the mobile grid gets a visible horizontal-scroll affordance: a "Swipe right →" hint plus a right-edge gradient fade on the shared `PositionGrid` container so the clipped columns are discoverable. Optionally default `showLetterNav` to true (one letter at a time) and add a sticky first column for extra orientation.
...
- **Status:** ✅ **Fixed (keep-list):** the table keeps `pagination={false}` (owner prefers one continuous scroll over click-to-page) with a `sticky` header, `fixed:"left"` `#`/`Name` columns, sortable headers, a mobile "Swipe right →" hint, and a right-edge gradient fade (shared `TableScrollHint`); the list layout is kept on mobile rather than transformed to cards, per the owner's row-comparison trade-off. **On the reason:** a dedicated "Reason" column was considered but the owner declined it — the **Note column already carries that context** and the reason is also encoded in the name's text colour, so an extra column would be redundant.
...
- **Fix (per owner — keep list on both breakpoints):** Keep the full list and rely on the table's **sticky header** in list mode so scanning works without paging (owner prefers scrolling over click-to-page); virtualize only if perf needs it. Sort **globally over all rows client-side** via the sortable column headers. On mobile the owner **keeps the list/table** (not stacked cards) because a shared row layout makes comparing rows and reading counts easier — the horizontal-scroll table is made navigable with `fixed:"left"` `#`/`Name` columns, a "Swipe right →" hint, and a right-edge gradient fade. The retirement reason stays surfaced via the existing **Note column** plus the name's colour coding (the owner declined a separate Reason column as redundant).
