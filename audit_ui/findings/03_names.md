...
- **Fix:** Reassign the retired-reason palette so green isn't reused for a retirement reason (e.g. use blue/teal for "Language Problem"), and avoid using `Flame` as the active→retired toggle since it also denotes "active" in `NameStatusIcon`. Document the status/reason color system in one shared legend.
