...
- **Status:** ✅ **Fixed (keep-list + Reason column):** a sortable **"Reason"** column was added to `RetiredNamesTable` (mapping `isLanguageProblem` 0–3 → "Destructive Storm" / "Language Problem" / "Misspelling" / "Special Storm", coloured via `getRetiredReasonColorClass`), so the filterable attribute is now visible in results. The table keeps `pagination={false}` (owner prefers one continuous scroll over click-to-page) with a `sticky` header, `fixed:"left"` `#`/`Name` columns, sortable headers, and the mobile "Swipe right →" hint; the list layout is kept on mobile rather than transformed to cards, per the owner's row-comparison trade-off.
