...
- **Fix (palette only — you're choosing colours anyway):** Flip the emphasis into a **diverging scale centred on the norm**: make `=6` a **neutral/muted grey** so it recedes, and give `<6` and `>6` the two accent colours so deviations pop — ideally a colour-blind-safe pair (e.g. teal `#0d9488` = faster, amber `#d97706` = slower) rather than green+red. The gap number is printed in each cell, so **no legend is required** (the rendered per-view description — see the "descriptions exist but unshown" finding — covers the meaning); an optional one-line caption ("colour = faster / slower than the ~6-year norm") is a nice-to-have. Optional guard: bucket on the same rounded value you display (`toFixed(2)`) so colour and label can never disagree on an edge case.
