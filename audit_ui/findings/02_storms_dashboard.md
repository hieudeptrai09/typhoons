...
### [Medium] The "Average" modal always says "Storm names at this position" — even for country / year / name groupings
- **Screens:** 68_modal_average; owner-provided shots of the "China" (country) vs "#21 Macao, China" (position) modals
- **Category:** Copy / Consistency / Architecture
- **Problem:** `AverageModal.tsx` hardcodes the label `Storm names at this position:` (line 57), but `DashboardPageContent.handleCellClick` opens this one component for Average-by-**Position**, **Country**, **Year**, and **Name**. The owner's screenshots confirm it: the country modal titled just "China" still reads "Storm names at this position," which is factually wrong (those are China's storms across many positions, not one naming slot). This is the visible cost of one merged modal serving several distinct groupings with fixed copy — and it overlaps `PositionModal`'s own Average tab, so the same "position average" content exists in two differently-designed modals.
- **Fix:** Make the copy context-driven — pass a `groupBy`/`subtitle` prop derived from the active filter ("Storm names for this country / year / name / position") instead of the hardcoded string. Whether you keep one parameterized component or split into dedicated per-context modals is the owner's call (see the modal-architecture note in the master report) and either is fine **as long as** each context gets correct copy — but if you split, keep the shared presentational row (name + count + colored avg) and header so the modals stay one visual family, and collapse the storms-⊂-average and `AverageModal`-vs-`PositionModal` redundancy rather than maintaining two designs.
