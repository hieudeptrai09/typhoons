...
> - **Intercept modal vs page:** confirmed the modal is intentionally a compact, contextual slice/peek of the page (often a single origin-driven tab) — that pattern stays, no dedicated modal needed. Residuals only: align the modal's shown fields/status with the page (incl. the retired badge/replacement), and stop the no-origin `PositionModal` fallback from omitting the Storms tab. The info modal will be redesigned to match the page (2-tab structure kept for brevity).
> - **Modal architecture (merge vs separate — owner deciding):** whether the intercept/average modals are one if/else component or dedicated per-case components is an implementation choice the visitor never sees — pick whatever keeps each modal *correct*. The current merge already leaks context (the "China" country modal wrongly says "Storm names at this position"). Recommendation: separating (or strictly parameterizing) is reasonable; **if you separate, still share the presentational primitives** (the name+count+colored-avg row, the header, the colors) so the modals stay one visual family, and **remove the redundancy** (storm tab ⊂ average tab; `AverageModal` overlaps `PositionModal`'s average) rather than maintaining two designs.
