...
> - **Tables & mobile parity (sorting):** sorting must always be **global over the full dataset** (client-side), independent of pagination. Owner prefers **scrolling over click-to-page**, so long list/table views keep the full list with a **sticky header** (no server pagination; virtualize only if needed) and any pagination must page the *pre-sorted* array. On mobile, sortable tables become **stacked cards with a sort control** (radio/segmented + asc/desc) that mirrors the desktop columns, so mobile keeps full sorting parity. Spatial position/country grids stay grids with a scroll affordance — the cards+sort pattern applies only to list/table views.
